...
\tred{
Massive multiple-input-multiple-output (MIMO) has emerged as a key technology for future 6G communication systems~\cite{busariMillimeterWaveMassiveMIMO2018}. To realize the system, accurate channel state information (CSI) is essential. In massive MIMO systems, the number of pilot symbols required for accurate CSI scales linearly with the number of antennas. However, the increasing number of pilot symbols leads to a severe pilot overhead bottleneck which in turn degrades spectral efficiency of the systems. Therefore, designing a channel estimation scheme that achieves high accuracy with significantly reduced pilot overhead is a critical research challenge.
}

\tred{
To address the pilot overhead bottleneck, early works proposed Compressive Sensing (CS)-based methods, which leverage the inherent sparsity of wireless channels in the angular domain~\cite{zhangAtomicNormDenoisingBased2018,mendez-rialHybridMIMOArchitectures2016,choiCompressedSensingWireless2017}. However, the performance of these methods is fundamentally limited by the model mismatch between the idealistic sparsity assumption and the characteristics of real-world channels. To overcome this limitation, data-driven deep learning (DL) approaches were introduced, which learn a direct mapping from the received pilot symbols to the channel estimate~\cite{heDeepLearningBasedChannel2018}. While effective, these supervised methods learn a measurement-specific mapping, making them highly dependent on the training conditions such as SNR and pilot structure. Consequently, they lack generalizability and require costly retraining when the measurement environment changes. A paradigm shift then emerged with deep generative models (DGMs), such as VAEs and GANs, which learn the underlying channel distribution itself to improve generalizability~\cite{vanhuynhGenerativeAIPhysical2024}. Nevertheless, these early DGMs have their own inherent limitations: VAEs often struggle with low expressiveness, while GANs are notoriously prone to training instability and mode collapse.
}

\tred{
Thus far, research on DM-based channel estimation has centered on a posterior sampling based on Bayesian inference framework~\cite{arvinteMIMOChannelEstimation2023,zhouGenerativeDiffusionModels2025}. The performance, therefore, highly depends on the sophisticated fusion of a data-driven prior and the likelihood from received pilot symbols. Leading approaches by~\cite{arvinteMIMOChannelEstimation2023,zhouGenerativeDiffusionModels2025} have explored stochastic and deterministic sampling, respectively. Both methods demonstrate superior performance over CS- and DGM-based methods under sufficient pilot conditions. However, they share a fundamental limitation where they rely on a statistically unsophisticated heuristic to derive the likelihood. This flaw creates an undesirable trade-off between accuracy and computational efficiency in the low-pilot regime. Arvinte's method compensates for this flawed guidance by employing thousands of sampling steps. This brute-force approach achieves high accuracy but suffers from prohibitive computational complexity~\cite{arvinteMIMOChannelEstimation2023}. Conversely, Zhou's method fails to fully leverage the learned prior due to uninformative prior. This leads to a sharp performance degradation when pilot overhead is low and a strong prior is most needed~\cite{zhouGenerativeDiffusionModels2025}.
}

% 서론이 appealing이 부족함
In this letter, a novel channel estimation method is proposed based on diffusion posterior sampling. This approach can provide higher estimation accuracy in both LOS and NLOS scenarios while jointly addressing pilot overhead and latency. The underlying idea is to efficiently approximate a intractable likelihood score. To achieve this, the diffusion posterior sampling is decomposed into a DM-based prior score and a likelihood score which can be formulated as a linear system of equations by leveraging moment matching principle and Tweedie's formula. Subsequently, the linear system is efficiently solved using a generalized minimal residual method (GMRES), thereby avoiding the high complexity of direct matrix inversion. From the simulation results, it is confirmed that the proposed method can achieve lower normalized mean square error (NMSE) than the state-of-the-art DM-based channel estimation methods under low pilot density in both LOS and NLOS environments. Furthermore, the proposed approach achieved a significantly lower computational complexity in terms of neural function evaluations (NFEs) compared to state-of-the-art (SOTA) DM-based methods.
...
\end{equation}
where $p(\mathbf{h}_{\text{a}})$, and $p(\mathbf{y}|\mathbf{h}_{\text{a}})$ are prior and likelihood distribution, respectively. To solve this problem, the DM-based posterior sampling is adopted.
...
\end{equation}
where $\epsilon_{t}\sim\mathcal{N}(\mathbf{0},\mathbf{I})$ is Gaussian noise, $\mathbf{h}_{0} = \mathbf{h}_{\text{a}}$, $\bar{\alpha}_{t}=\prod_{i=1}^{t}\alpha_{i}$, $\alpha_{t}=1-\beta_{t}$, and $\beta_{t}$ is pre-defined variance schedule. 
...
\begin{equation}
p_{t}(\mathbf{y}|\mathbf{h}_{t}) = \int p(\mathbf{y}|\mathbf{h}_{0})p_{t}(\mathbf{h}_{0}|\mathbf{h}_{t})d\mathbf{h}_{0}.
\end{equation}
Even in this form, the likelihood $p_{t}(\mathbf{y}|\mathbf{h}_{t})$ is still intractable because the true distribution $p_{t}(\mathbf{h}_{0}|\mathbf{h}_{t})$ of the likelihood is unknown. To address this intractability, we approximate the likelihood to tractable Gaussian distribution $q_{t}(\mathbf{h}_{0}|\mathbf{h}_{t})$ as~\cite{arvinteMIMOChannelEstimation2023,zhouGenerativeDiffusionModels2025},
\begin{equation}
p_{t}(\mathbf{y}|\mathbf{h}_{t}) \approx \int p(\mathbf{y}|\mathbf{h}_{0})q_{t}(\mathbf{h}_{0}|\mathbf{h}_{t})d\mathbf{h}_{0}.
\end{equation}
...
Given the well-approximated prior score $\nabla_{\mathbf{h}_{t}}\log p_{t}(\mathbf{h}_{t})$ from the DM, the first and second moments of the true distribution can be, respectively, approximated using Tweedie's formula as~\cite{efronTweediesFormulaSelection2011},
\begin{equation}
...
\end{equation}
where $\boldsymbol{\Sigma}_{t} = \text{diag}((1-\bar{\alpha}_{t}) / \sqrt{ \bar{\alpha}_{t} })$. Through this process, the approximated likelihood and its score can be, respectively, derived as,
\begin{equation}
...
(\boldsymbol{\Sigma}_{\mathbf{n}}+\mathbf{A}\mathbb{V}[\mathbf{h}_{0}|\mathbf{h}_{t}]\mathbf{A}^{\top})\mathbf{u} = \mathbf{y}- \mathbf{A}\mathbb{E}[\mathbf{h}_{0}|\mathbf{h}_{t}].
\end{equation}

The proposed method is summarized in Algorithm~\ref{alg:algorithm1}.
...
\FOR{$t=T$ to $1$}
	...
	\STATE Compute posterior score $\nabla_{\mathbf{h}_{t}}\log p_{t}(\mathbf{h}_{t}|\mathbf{y})$ via~\eqref{eq:posterior_score}
	\STATE Sample $\mathbf{h}_{t-1}$ using the posterior score via~\eqref{eq:posterior_sampling}
\ENDFOR
\STATE $\hat{\mathbf{h}} = ((\mathbf{A}_{\text{T}}^{H})^{\top}\otimes \mathbf{A}_{\text{R}})\mathbf{h}_{0}$
\RETURN $\hat{\mathbf{h}}$
\end{algorithmic}
\end{algorithm}
